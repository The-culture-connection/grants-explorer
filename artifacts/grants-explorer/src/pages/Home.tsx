...
function ProfileModal({ onClose }: { onClose: () => void }) {
  const { user, updateProfile, linkGoogle, logout } = useAuth();
  ...
  async function handleLinkGoogle() {
    ...
    try {
      await linkGoogle();
      setLinkMsg("Google account linked successfully!");
    } catch (err: any) {
      setLinkMsg(err.message || "Failed to link Google account.");
    } finally {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-background border border-border rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-2">
            {!user?.hasGoogle && (
              <Button variant="outline" size="sm" className="text-xs gap-1.5 h-7 border-border/60" onClick={handleLinkGoogle} disabled={linkingGoogle}>
                {linkingGoogle
                  ? <><span className="w-3 h-3 border border-muted-foreground/50 border-t-transparent rounded-full animate-spin" /> Linking…</>
                  : <>
            ...
            {user?.hasGoogle && (
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
                <svg className="w-3 h-3" viewBox="0 0 24 24"><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/></svg>
                Google linked
              </span>
            )}
        ...
        {linkMsg && (
          <div className={`px-5 py-2 text-xs border-b ${linkMsg.includes("success") ? "text-emerald-700 bg-emerald-50 border-emerald-200 dark:bg-emerald-950/30 dark:border-emerald-800 dark:text-emerald-400" : "text-red-600 bg-red-50 border-red-200 dark:bg-red-950/30 dark:border-red-800"}`}>
            {linkMsg}
          </div>
        )}
