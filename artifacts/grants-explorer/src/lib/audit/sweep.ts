...
// ─── V4 Consolidated Sweep ────────────────────────────────────────────────────

export interface V4SweepResult {
  stats: SweepStats;
  topTraces: V4ScoreTrace[];
}

export function runV4Sweep(
  org: OrgProfile,
  pool: NormalizedOpportunity[],
  topN = 50,
): V4SweepResult {
  ...
  const stats: SweepStats = {
    ...
    dimensionSums: {
      conceptFit: 0, conceptCentrality: 0, activityFit: 0, populationFit: 0,
      targetApplicantFit: 0, eligibility: 0, geographyFit: 0, capacityFit: 0,
      fundingFit: 0, sourceRelevance: 0,
    },
    dimensionAvgs: {},
    riskCounts: {},
    semanticBoostTotal: 0,
  ...
  for (const opp of active) {
    ...
    const trace = scoreMatchV4(org, opp);
